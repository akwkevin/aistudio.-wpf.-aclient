...
{
    class D_UserMessageDocumentRenderer : IDocumentRenderer
    {
        public void Render(FlowDocument doc, object data)
        {
            if (data is ObservableCollection<D_UserMessageDTO> items)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    cell = new TableCell(new Paragraph(new Run(item.GroupId.ToString())));
                    cell.Style = styleCell;
                    row.Cells.Add(cell);
