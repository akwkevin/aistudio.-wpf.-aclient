...
namespace AIStudio.Wpf.EFBusiness
{
    ...
    public abstract class BaseBusiness<T> : IBaseBusiness<T> where T : class, new()
    {
        ...
        /// 注：仅支持单线程操作
        /// </summary>
        public IDbAccessor Db { get { return EFCoreDataProviderExtension.ServiceProvider.GetRequiredService<IDbAccessor>(); } }
        ...
        {
            return Db.GetIQueryable<T>().FirstOrDefault<T>(predicate);
        }
        ...
        {
            return await Db.GetIQueryable<T>().FirstOrDefaultAsync<T>(predicate);
        }
