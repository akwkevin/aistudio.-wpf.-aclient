...
namespace AIStudio.Wpf.EFBusiness
{
    ...
    public abstract class ShardingBaseBusiness<T> : IShardingBaseBusiness<T> where T : class, new()
    {
        #region 构造函数
        ...
        public ShardingBaseBusiness(IShardingDbAccessor shardingDb, IDbAccessor db)
        {
        ...
        /// 注：仅支持单线程操作
        /// </summary>
        public IShardingDbAccessor ShardingDb { get; }
        ...
        {
            return ShardingDb.RunTransaction(action, isolationLevel);
        }
        ...
        {
            return await ShardingDb.RunTransactionAsync(action, isolationLevel);
        }
        ...
        {
            return ShardingDb.Insert(entity);
        }
        ...
        {
            return await ShardingDb.InsertAsync(entity);
        }
        ...
        {
            return ShardingDb.Insert(entities);
        }
        ...
        {
            return await ShardingDb.DeleteAllAsync<T>();
        }
        ...
        {
            return ShardingDb.Delete<T>(entity);
        }
        ...
        {
            return await ShardingDb.DeleteAsync(entity);
        }
        ...
        {
            return ShardingDb.Delete<T>(entities);
        }
        ...
        {
            return await ShardingDb.DeleteAsync<T>(entities);
        }
        ...
        {
            return ShardingDb.Delete(condition);
        }
        ...
        {
            return await ShardingDb.DeleteAsync(condition);
        }
        ...
        {
            return ShardingDb.DeleteSql(where);
        }
        ...
        {
            return await ShardingDb.DeleteSqlAsync(where);
        }
        ...
        {
            return ShardingDb.Update(entity);
        }
        ...
        {
            return await ShardingDb.UpdateAsync(entity);
        }
        ...
        {
            return ShardingDb.Update(entities);
        }
        ...
        {
            return ShardingDb.Update(whereExpre, set);
        }
        ...
        {
            return await ShardingDb.UpdateAsync(whereExpre, set);
        }
        ...
        {
            return ShardingDb.GetIShardingQueryable<T>().Where(predicate).FirstOrDefault();
        }
        ...
        {
            return await ShardingDb.GetIShardingQueryable<T>().Where(predicate).FirstOrDefaultAsync();
        }
        ...
        {
            return ShardingDb.GetIShardingQueryable<T>().ToList();
        }
        ...
        {
            return await ShardingDb.GetIShardingQueryable<T>().ToListAsync();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IShardingQueryable<T> GetIQueryable()
        {
            return ShardingDb.GetIShardingQueryable<T>();
        }
        ...
        /// <returns></returns>
        public async Task<List<SelectOption>> GetOptionListAsync(OptionListInputDTO input, string textFiled, string valueField, IShardingQueryable<T> source = null)
        {
            ...
            List<T> newQList = await GetNewQ().Where(where, ids, input.q).OrderBy($@"{pageInput.SortField} {pageInput.SortType}")
        ...
        #region 历史数据查询
        public IShardingQueryable<T> GetHistoryDataQueryable(Expression<Func<T, bool>> expression, DateTime? start, DateTime? end, string dateField = "CreateTime")
        {
            ...
            return ShardingDb.GetIShardingQueryable<T>().Where(expression).OrderBy(dateField);
        }
        public async Task<int> GetHistoryDataCount(Expression<Func<T, bool>> expression, DateTime? start, DateTime? end, string dateField = "CreateTime")
        {
            ...
            return await ShardingDb.GetIShardingQueryable<T>().Where(expression).OrderBy(dateField).CountAsync();
        }
        ...
        public async Task<PageResult<T>> GetPageHistoryDataList(PageInput pageInput, Expression<Func<T, bool>> expression, DateTime? start, DateTime? end, string dateField = "CreateTime")
        {
            ...
            var queryable = ShardingDb.GetIShardingQueryable<T>().Where(expression);

            int count = await queryable.CountAsync();
            var list = await queryable.Where(expression).OrderBy($@"{pageInput.SortField} {pageInput.SortType}")
            .Skip((pageInput.PageIndex - 1) * pageInput.PageRows)
            .Take(pageInput.PageRows)
            .ToListAsync();
