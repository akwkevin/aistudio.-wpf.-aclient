...
namespace AIStudio.Wpf.DataRepository
{
    internal class GenericDbAccessor : DefaultDbAccessor
    {
        ...
        public GenericDbAccessor(string conString, DatabaseType dbType, ICollection<TableMapperRule> rules)
        {
        ...
        private string GetFormatedSchemaAndTableName(Type entityType)
        {
            ...
            if (Rules != null)
            {
                var rule = Rules.FirstOrDefault(p => p.MappingType == entityType);
                if (rule != null)
                {
                    table += $"_{rule.TableSuffix}";
                }
            }
        ...
        public override DatabaseType DbType { get; }

        public ICollection<TableMapperRule> Rules { get; }
        ...
        public override async Task<DataTable> GetDataTableWithSqlAsync(string sql, params (string paramterName, object value)[] parameters)
        {
            ...
            using (var reader = await conn.ExecuteReaderAsync(
                sql, CreateDynamicParameters(parameters), _transaction?.GetDbTransaction(), CommandTimeout))
            {
        ...
        public override async Task<List<T>> GetListBySqlAsync<T>(string sql, params (string paramterName, object value)[] parameters)
        {
            ...
            return (await conn.QueryAsync<T>(sql, CreateDynamicParameters(parameters), _transaction?.GetDbTransaction(), CommandTimeout)).ToList();
        }
